            ...
            <ScrollView className="flex-1 px-4 pt-4">
                            ...
                            <View key={index} className={`flex-row ${index !== order.items.length - 1 && 'border-b border-gray-100 pb-4 mb-4'}`}>
                                ...
                                <View className="flex-1 ml-3 justify-center">
                                    ...
                                    <View className="flex-row justify-between items-center mt-2">
                                        <Text className="text-primary font-bold">NPR {item.price}</Text>
                ...
                <View className="bg-white p-4 rounded-xl mb-8 border border-gray-100">
                    ...
                    <View className="flex-row justify-between mb-2">
                        ...
                        <Text className="text-primary font-medium">NPR {order.subtotal.toFixed(2)}</Text>
                    </View>
                    <View className="flex-row justify-between mb-2">
                        <Text className="text-secondary">Shipping</Text>
                        <Text className="text-primary font-medium">NPR {order.shippingCost.toFixed(2)}</Text>
                    </View>
                    <View className="flex-row justify-between mb-2">
                        <Text className="text-secondary">Tax</Text>
                        <Text className="text-primary font-medium">NPR {order.tax.toFixed(2)}</Text>
                    </View>
                    <View className="h-px bg-gray-100 my-2" />
                    <View className="flex-row justify-between">
                        <Text className="text-primary font-bold text-lg">Total</Text>
                        <Text className="text-primary font-bold text-lg">NPR{order.totalAmount.toFixed(2)}</Text>
                    </View>
                </View>
            </ScrollView>
        </SafeAreaView>
